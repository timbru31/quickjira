...
const saveOptions = event => {
  ...
  if (!urlPattern.test(jira)) {
    status.textContent = browser.i18n.getMessage('validURL');
  } else {
    let defaultOption = document.querySelector('select').value;
    // Map currentTab to 0 and newTab to 1
    if (browser.i18n.getMessage('currentTab') === defaultOption) {
      defaultOption = 0;
    } else {
      defaultOption = 1;
    }
    storage.set({
      ...
      // notify user
      status.textContent = browser.i18n.getMessage('savedOptions');
      // remove after 500ms
      window.setTimeout(() => {
        window.close();
      }, 1000);
...
const restoreOptions = () => {
  document.querySelector('.quiji-options-save').value = browser.i18n.getMessage('saveOptions');
  ...
  storage.get({
    ...
    let defaultOption = browser.i18n.getMessage('currentTab');
    if (options && options.defaultOption === 1) {
      defaultOption = browser.i18n.getMessage('newTab');
    }
